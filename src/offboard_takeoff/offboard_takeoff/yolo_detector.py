"""ROS 2 YOLO detector node using OpenCV DNN and an ONNX model."""
...
class YoloDetector(Node):
    """Detect COCO objects from a ROS image topic with OpenCV DNN."""
    ...
    def _resolve_model_path(self, model_path: str) -> Path:
        """Resolve and validate the ONNX model path."""

        if not model_path:
            raise ValueError(
                'Parameter "model_path" is empty. '
                'Provide a path to a YOLO ONNX model file.'
            )

        resolved_path = Path(model_path).expanduser()
        if not resolved_path.is_file():
            raise FileNotFoundError(
                f'YOLO model file was not found: {resolved_path}'
            )
    ...
    def _initialize_inference_backend(self):
        ...
        if backend not in {'auto', 'onnxruntime', 'opencv'}:
            raise ValueError(
                ...
                'Use one of: auto, onnxruntime, opencv.'
            )
    ...
    def _load_class_names(self) -> list[str]:
        ...
        class_names = [str(name).strip() for name in parameter_value]
        if not class_names:
    ...
    def _image_callback(self, msg: Image):
        ...
        if not self._should_publish_debug_image():
            if self.show_debug_window:
                ...
                cv2.imshow('YOLO Debug View', debug_frame)
                cv2.waitKey(1)
            return
    ...
    def _run_inference(self, frame: np.ndarray) -> list[Detection]:
        ...
        for row in predictions:
            class_id, confidence, box = self._decode_prediction_row(
                row=row,
                letterbox=letterbox,
    ...
    def _draw_fps_overlay(self, frame: np.ndarray):
        ...
        text = f'YOLO FPS: {self.smoothed_fps:.1f}'
    ...
    def _forward(
        ...
        try:
            ...
            if not self.forward_error_logged:
                self.get_logger().error(
                    'YOLO forward failed. This ONNX model is likely exported '
                    'for a different fixed input size than the current '
                    f'{self.input_width}x{self.input_height}. '
                    'Try launching with the original export size, for example '
                    '640x640, or re-export the model directly at the smaller '
    ...
    def _draw_detections(
        ...
        """Draw YOLO boxes and labels on a frame."""

        for detection in detections:
            is_target = detection.label.lower() in self.target_labels_set
            color = (0, 0, 255) if is_target else (0, 180, 255)
            label = (
                f'PERSON {detection.confidence:.2f}'
                if is_target
                else f'{detection.label} {detection.confidence:.2f}'
            )
